...
void Sprite::Render()
{
	if (m_Parent)
	{
		if (m_Parent->m_Tag == "Player") {
			SetRect(&m_Parent->m_Collision,
				(m_Parent->m_Position.x - (m_Size.x * 1) / 2),
	...
	{
		SetRect(&m_Collision, m_Position.x - (m_Size.x * m_Scale.x) / 2, m_Position.y -(m_Size.y * m_Scale.y)/ 2,
			m_Position.x + (m_Size.x * m_Scale.x) / 2, m_Position.y + (m_Size.y * m_Scale.y) / 2);
	}
